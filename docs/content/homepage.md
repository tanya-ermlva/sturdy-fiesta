# Homepage Copy

Last updated: 2026-03-01
Status: in progress — fill in [brackets] before building

---

## Name

Tanya Ermolaeva

---

## Bio Lines

One thought per line, declarative. Format nods to Granola's meeting summaries.
All six lines carry a footnote number that expands on hover.

```
Moscow-born product & visual designer (1)
Based in London since 2023 (2)
Currently designing at Granola (3)
Previously at Intercom, Wednesday Studio, Aaply, and Strelka Institute (4)
Generalist background: product, visual, branding, and motion (5)
Increasingly building as well as designing (6)
```

---

## Footnotes

Expand on hover. Tone: informative, friendly — not too playful.

**(1)** Studies philosophy and design in Moscow. An unlikely pairing that made complete sense. Left in 2022

**(2)** Moved to London in 2023.

**(3)** Granola is an AI meeting assistant. I work on the core product experience.

**(4)** From founding designer roles in small teams to working at larger companies, from studios to in-house work.

**(5)** Started in visual and brand, moved into product. The motion work happened
somewhere in the middle and never fully stopped.

**(6)** This site is the experiment.

---

## Social Links

Top-right, stacked vertically. Plain text, no icons.

- Email — [your email]
- LinkedIn — [url]
- Instagram — [url]
- Twitter — [url, or remove if inactive]

---

## Open questions

- [ ] Footnote (1): which school, and which year did you leave Moscow?
- [ ] Footnote (2): which year did you move to London? Any London line, or drop it?
- [ ] Social links: confirm all four are active
